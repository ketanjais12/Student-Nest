...
const MyHostels = () => {
  ...
  if (loading)
    return (
      <div className="pt-32 text-center text-xl font-bold text-gray-500">
        ...
        {hostels.length === 0 ? (
          ...
          <div className="grid gap-6">
            {hostels.map((hostel) => (
              <div
                key={hostel._id}
                className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-6 items-center"
              >
                <div className="w-full md:w-48 h-32 bg-gray-200 rounded-xl overflow-hidden shrink-0">
                  {hostel.images && hostel.images.length > 0 ? (
                    <img
                      src={
                        hostel.images[0].includes("cloudinary.com") ||
                        hostel.images[0].startsWith("http")
                          ? hostel.images[0].replace("https//", "https://")
                          : `https://student-nest-bjtk.onrender.com/${hostel.images[0].replace(/\\/g, "/").replace(/^\//, "")}`
                      }
                      alt={hostel.name}
